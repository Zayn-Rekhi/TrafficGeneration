...
class VectorQuantizer(nn.Module):
    ...
    def __init__(self, opts, device):
        ...
        self.n_e = opts['n_embeddings']
        self.e_dim = opts['latent_dim']
        self.beta = opts['beta']

        self.embedding = nn.Embedding(self.n_e, self.e_dim)
        self.embedding.weight.data.uniform_(-1.0 / (self.e_dim ** 0.5), 1.0 / (self.e_dim ** 0.5))
...
class BlockGenerator(torch.nn.Module):
    def __init__(self, opt, device):    
        ...
        self.vector_quantization = VectorQuantizer(opt, self.device)
    ...
    def forward(self, data):
        z = self.encode(data)
        z_norm = self.pre_vq(z)
        z, loss, perplexity = self.vector_quantization(z_norm)
        ...
        return pos, size, vel, acttype, direc, laneidx, loss, perplexity
