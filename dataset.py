...
class TrafficDataset(Dataset):
    ...
    def load_data(self):
        ...
        for folder in ls_folders:
            data = self._load_data(os.path.join(self.data_root, folder))
    ...
    def construct_graph_from_data(self, data):
        ...
        for actor in actors:
            ...
            graph.add_node(curr_idx, 
                           ...
                           direction=self._encode_direction(actor['direction']))
        ...
        processed = graph2vector_processed(graph)
        node_pos, actor_type, lane_index, direction, edge_list, node_idx = processed       


        obj = CustomData(edge_index=torch.tensor(edge_list, dtype=torch.int64), 
    ...
    def _encode_direction(self, x):
        return [self.map_direction.get(x, 0)]
...
if __name__ == "__main__":
    ...
    data_root = "/home/zayn/dataset_processed_traffic/highD-dataset-v1.0"
